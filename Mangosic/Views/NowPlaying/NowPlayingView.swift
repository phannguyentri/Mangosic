    ...
    private var playlistHeader: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                // Show playlist name if playing from a playlist
                if let playlistName = queueService.currentPlaylistName {
                    HStack(spacing: 6) {
                        Image(systemName: "music.note.list")
                            .font(.caption)
                            .foregroundColor(Theme.primaryEnd)
                    ...
                    Text(queueService.count == 1 ? "Single Track" : "\(queueService.count) tracks")
                        .font(.subheadline)
                        .foregroundColor(.gray)
                }
